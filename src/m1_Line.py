...
class Line(object):
    ...
    def line_plus(self, other_line):
        ...
        newstartx=self.start.x + other_line.start.x
        newstarty=self.start.y + other_line.start.y
        newendx = self.end.x + other_line.end.x
        newendy = self.end.y + other_line.end.y
        return Line(Point(newstartx,newstarty),Point(newendx,newendy))
    ...
    def is_parallel(self, line2):
        ...
        slope1=self.slope()
        slope2=line2.slope()
        if abs(round(slope1,12)) == abs(round(slope2,12)):
            return True
        else:
            return False
